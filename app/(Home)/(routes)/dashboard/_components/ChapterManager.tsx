...
import { Chapter } from "../../../../../types/hygraph";
...
function ChapterForm({ chapter, onSave, onCancel, isSaving, itemId }: ChapterFormProps) {
...
export default function ChapterManager({ itemId, chapters = [], onRefresh }: ChapterManagerProps) {
  ...
  // ── CREATE ────────────────────────────────────────────────────────────────
  const handleCreate = async (data: Partial<Chapter>) => {
    setIsSaving(true);
    setError("");
    const optimisticId = `optimistic-${Date.now()}`;
    // Show immediately
    setLocalChapters((prev) => [...prev, { id: optimisticId, ...data } as Chapter]);
    setIsAdding(false);
    try {
  ...
  // ── UPDATE ────────────────────────────────────────────────────────────────
  const handleUpdate = async (chapterId: string, data: Partial<Chapter>) => {
    setIsSaving(true);
    setError("");
