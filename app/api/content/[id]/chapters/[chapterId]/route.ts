...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string, chapterId: string }> }) {
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string, chapterId: string }> }) {
  const { id: parentId, chapterId } = await params;
  try {
